...
async function init() {
  ...
  async function commit() {
    ...
    // API to see if it's a valid word
    isLoading = true;
    setLoading(isLoading);
    ...
    // first pass finds correct letters so we can mark those as correct first
  ...
  // listening for event keys and routing to the right function , we listen on keydown so we can catch Enter and Backspace
...
// function to check to see if a character is alphabet letter
...
// an object out of it (like {E: 2, L: 1, T: 1}) so we can use that
